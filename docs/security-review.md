...
- Verification: Helm render tests assert the Restricted controls and the Dockerfile pins `python:3.12.14-slim-bookworm@sha256:0f5b26...`. Post-merge GitHub Actions run `33403240549` passed the blocking HIGH/CRITICAL scan, published SBOM/provenance attestations and the multi-platform manifest `sha256:22c30e...`; an anonymous GHCR token exchange returned HTTP 200 and the same digest. The chart selects that digest by default. In a namespace enforcing `restricted:latest`, the API rejected a privileged negative-control pod and admitted the actual digest-pinned event-sink Deployment without Pod Security warnings.
